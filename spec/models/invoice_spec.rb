...
RSpec.describe Invoice do
  ...
  describe 'instance methods' do
    ...
    it 'applies only 1 max discount' do
      @merchant = Merchant.create!(name: 'Sally Handmade')
      ...
      @item_1 = @merchant.items.create!(name: 'Qui Essie', description: 'Lorem ipsim', unit_price: 100)
      @item_2 =  @merchant.items.create!(name: 'Essie', description: 'Lorem ipsim', unit_price: 100)

      @invoice = Invoice.create!(customer_id: @customer.id, status: 'completed')

      @invi_1 = InvoiceItem.create!(item_id: @item_1.id, invoice_id: @invoice.id, quantity: 30, unit_price: 100, status: 1)

      expect(@invoice.total_of_discount).to eq(900)
    end
  end
end
